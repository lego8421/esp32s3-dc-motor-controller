...
void ATServer::register_help_command(void)
{
    ...
    tmp.write_handler_args_list = std::string("");
    ...
    const char *write_handler_args_list)
{
    ...
    if (write_handler_args_list != nullptr) {
        temp_cmd.write_handler_args_list = std::string(write_handler_args_list);
    }
    ...
    const char *write_handler_args_list)
{
    for (auto it = registered_commands.begin(); it != registered_commands.end(); ++it) {
        if (it->command.compare(cmd) == 0) {
            ...
            if (write_handler_args_list != nullptr) {
                it->write_handler_args_list = std::string(write_handler_args_list);
            }
            return true;
        }
    }
...
void ATServer::handle(char c)
{
    ...
    static std::vector<char> control_sequence;
...
bool ATServer::execute(std::string &input)
{
